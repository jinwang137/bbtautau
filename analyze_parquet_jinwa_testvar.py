...
def plot_distributions(df, output_dir="plots"):
    ...
    # FatJet matching
    if 'SubJetPhi' in df.columns:
        plot_count += 1
        plt.subplot(1, 1, plot_count)
        if len(df['SubJetPhi'].shape) > 1:
            data = df['SubJetPhi'].iloc[:, 0]
        else:
            data = df['SubJetPhi']
        ...
        plt.xlabel('SubJetPhi (1st FatJet)')
        plt.ylabel('Events')
        plt.title('SubJetPhi (1st FatJet)')
    
    
    plt.tight_layout()
    plt.savefig(f'{output_dir}/quick_sub.png', dpi=150, bbox_inches='tight')
    print(f"Plot saved to {output_dir}/quick_sub.png")
    plt.show()
